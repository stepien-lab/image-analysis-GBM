    ...
    def __init__(self,
        ...
        if domain is None:
            ...
            if np.shape(self.domain)[0] != self.dimension:
                raise RuntimeError(
                    'Specified domain should be nDimensions by 2, specifying domain min and max values in each dimension')
            ...
            for d in range(self.dimension):
                if self.domain[d, 0] >= self.domain[d, 1]:
                    raise RuntimeError(
                        f'Specified domain minimum value in dimension {d} ({self.domain[d, 0]}) must be lower than maximum value ({self.domain[d, 1]})')
        if self.dimension == 2:
    ...
    def addLabels(self, labelName, labelType, labels, cmap=None):
        ...
        if labelType in ['categorical', 'continuous']:
            ...
            if labelType == 'categorical':
                ...
                self.labels[labelName]['numericalLabels'] = np.asarray(
                    [labelToInteger[labels[v]] for v in range(len(labels))])
                if cmap is None:
                    # Use default colormap
                    cmap = 'tab10'
                self.labels[labelName]['integerToColor'] = {v: plt.cm.get_cmap(cmap)(v) for v in
                                                            self.labels[labelName]['integerToLabel'].keys()}
...
def visualisePointCloud(pc, labelForVisualisation=None, cmap=None, markerSize=None, vmin=None, vmax=None):
    ...
    if labelForVisualisation is None:
        ...
        plt.scatter(pc.points[shuffleOrder, 0], pc.points[shuffleOrder, 1],
                    c=pc.labels[labelForVisualisation]['numericalLabels'][shuffleOrder], s=markerSize, cmap=cmap,
...
def weightedPairCorrelationFunction(pc, categoricalLabelName, categoricalLabelToPlot, continuousLabelName, maxR=0.5,
                                    annulusWidth=0.025, annulusStep=0.025, targetP=None, weightingFunction=None):
    # First we check that the chosen label is categorical
    ...
    l_B = pc.labels[continuousLabelName]['numericalLabels'][
        ~np.isnan(pc.labels[continuousLabelName]['numericalLabels'])]
...
def neighbourhoodCorrelationFunction(pc, labelName, categoriesToPlot, maxR=0.5):
    ...
    for i in range(nCandidates):
        (center_x, center_y, radius) = make_circle(
            [pointsToCompare[v][allCandidates[i, v]] for v in range(nCategories)])
        if radius < maxR:
...
def topographicalCorrelationMap(pc, labelNameA, labelA, labelNameB, labelB, radiusOfInterest=0.1,
    ...
    # Now map to [-1,1]
    transformedMarks[transformedMarks < 0] = (transformedMarks[transformedMarks < 0] + 1) / (
                maxCorrelationThreshold - 1)
    transformedMarks[transformedMarks > 0] = (transformedMarks[transformedMarks > 0] - 1) / (
                maxCorrelationThreshold - 1)
    ...
    if visualiseStages:
        ...
        fig, ax = plotTopographicalCorrelationMap(pc, topographicalCorrelationMap.T, ax=None, cmap='RdBu_r',
                                                  colorbarLimit=l)
...
def plotTopographicalCorrelationMap(pc, topographicalCorrelationMap, ax=None, cmap='RdBu_r', colorbarLimit=None):
    ...
    im = ax.imshow(topographicalCorrelationMap, origin='lower', cmap=cmap, extent=extent, vmin=-colorbarLimit,
                   vmax=colorbarLimit)
...
def getAnnulusAreasAroundPoints(points_i, maxR, annulusStep, annulusWidth, domain):
    ...
    domainX = domain[0, :]
    domainY = domain[1, :]
    vfunc_returnAreaOfCircleInDomainAroundPoint = np.vectorize(returnAreaOfCircleInDomainAroundPoint,
                                                               excluded=['points', 'domainX', 'domainY'])
    PCF_radii_lower = np.arange(0, maxR + annulusStep, annulusStep)
    PCF_radii_upper = np.arange(annulusWidth, maxR + annulusWidth + annulusStep, annulusStep)
    ...
    for annulus in range(len(PCF_radii_lower)):
        ...
        areas_in = vfunc_returnAreaOfCircleInDomainAroundPoint(index=np.arange(len(points_i)), points=points_i, r=inner,
                                                               domainX=domainX, domainY=domainY)
        areas_out = vfunc_returnAreaOfCircleInDomainAroundPoint(index=np.arange(len(points_i)), points=points_i,
                                                                r=outer, domainX=domainX, domainY=domainY)
...
def getAnnulusAreasAroundPoints_polygon(points_i, maxR, annulusStep, annulusWidth, polygon):
    # We want to populate a table the same size as distances, which contains the area of the annulus containing that contribution
    # i.e., "at distance D(i->j) from point i, what is area of containing annulus?"
    vfunc_returnAreaOfCircleInDomainAroundPoint = np.vectorize(returnAreaOfCircleInDomainAroundPoint_polygon,
                                                               excluded=['points', 'r', 'polygon'])
    PCF_radii_lower = np.arange(0, maxR + annulusStep, annulusStep)
    PCF_radii_upper = np.arange(annulusWidth, maxR + annulusWidth + annulusStep, annulusStep)
...
def returnIntersectionPoints(x0, y0, r, domainX, domainY):
    ...
    temp = [[np.max([np.min([point[0], domainX[1]]), domainX[0]]), np.max([np.min([point[1], domainY[1]]), domainY[0]])]
